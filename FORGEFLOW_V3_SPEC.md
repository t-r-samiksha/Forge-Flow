...
| 4 | Red Team Arena → Redcap | Dynamic attack generation + real LLM judgment | ⏳ Not started — current arena is 5 hardcoded prompts, keyword substring matching, not model judgment |
...
LYZR_REDCAP_AGENT_ID=    # pending -- create in Lyzr Studio for Phase 4 (Red Team)
```
...
- Red Team Arena: 5 hardcoded prompts, keyword-substring judging, no Redcap
...
After 2e: Phase 4 -- Red Team Arena -> Redcap. Still needed: Redcap provisioned in Lyzr Studio, `agent_id` added as `LYZR_REDCAP_AGENT_ID`.
